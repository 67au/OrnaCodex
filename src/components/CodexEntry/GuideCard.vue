  ...
  <var-card class="card" title="OrnaGuide" v-if="isNotEmpty">
    <template #description>
      <div class="card-description">
        <template v-for="[status, element] in (Object.entries(data?.elements || []) as Array<[string, string[]]>)">
        ...
        <var-cell v-if="data?.statusImmunities" class="guide-cell" border>
          <var-space align="center" size="small" class="leading-none">
            ...
            <var-chip type="warning" size="small" v-for="status, index in data.statusImmunities" :round="false" plain
              :key="index">
        ...
        <var-cell v-if="data?.spawns" class="guide-cell" border>
          <var-space align="center" size="small" class="leading-none">
            {{ $t('guide.spawns') + ':' }}
            <var-chip type="primary" size="small" v-for="spawn in data.spawns" :round="false" plain :key="spawn">
              {{ spawn }}
            </var-chip>
          </var-space>
        </var-cell>
        <var-cell v-if="data?.element" class="guide-cell" border>
          <var-space align="center" size="small" class="leading-none">
            {{ $t('guide.element') + ':' }}
            <var-chip :color="global.elementColor[data.element.toLowerCase()]" size="small" :round="false" plain>
